...
REAL_BULLETED = """- 30 new cases in week 15 (8 - 14 April 2019) [5].
- 41 new cases in week 16 (15 - 21 April 2019) [3].
- 133 new cases in week 22 (27 May - 2 June 2019) [1].

The extracts do not provide a cumulative total for Banadir alone; they only \
give cumulative totals for all five regions combined."""


def test_bulleted_answer_scores_full_coverage():
    """Each bullet is its own claim.

    The sentence rule needs a capital after the full stop, so it never fired
    between "…April 2019) [5]." and "- 41 new cases…". Six cited bullets
    counted as one unit and coverage was meaningless.
    """
    v = verify_answer(REAL_BULLETED, n_sources=6)
    assert v.citation_coverage == 1.0
    assert v.cited_markers == [1, 3, 5]
    assert v.ok


def test_naming_what_the_sources_lack_is_not_an_uncited_claim():
    """The caveat we most want is a statement about the extracts, not from them."""
    v = verify_answer(REAL_BULLETED, n_sources=6)
    assert not v.uncited_figure_sentences


@pytest.mark.parametrize("marker", ["[1]", "\u30101\u3011", "\uff3b1\uff3d"])
def test_bracket_variants_count_as_citations(marker):
    """gpt-oss emitted 【1】; a regex that only knew [1] scored it 0%."""
    v = verify_answer(f"The rate was 27,000 shillings per dollar as of 3 May 2020{marker}.", 3)
    assert v.cited_markers == [1] and v.ok


def test_widened_refusal_pattern_does_not_blunt_the_figure_check():
    """Broadening what counts as meta must not let a bare figure through."""
    v = verify_answer("An estimated 3.8 million people remained internally displaced.", 3)
    assert not v.ok and v.uncited_figure_sentences


def test_unicode_dashes_do_not_hide_dates():
    """gpt-oss wrote 2019‑03‑10 with U+2011, and the audit reported no dates."""
    text = ("Cumulative cases were 6,899 as of 2019\u201103\u201110 [1] and "
            "7,063 as of 2019\u201104\u201114 [2].")
    v = verify_answer(text, 3)
    assert "2019-03-10" in v.dates_mentioned
    assert "2019-04-14" in v.dates_mentioned
    assert v.ok


def test_figures_quoted_inside_a_refusal_still_need_citing():
    """A partial refusal that quotes numbers is still quoting numbers."""
    text = ("The extracts do not provide a Banadir-only count. The cumulative "
            "figures given (6,899 and 7,063) are totals for five regions.")
    v = verify_answer(text, 3)
    assert v.uncited_figure_sentences and not v.ok
